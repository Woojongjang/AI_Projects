...
def getPath(startingPrime, finalPrime, depth, visited = set(), path = [], layer = 0):
    if startingPrime == finalPrime:
        ...
        for prime in getPossibleActions(startingPrime):
            if prime not in visited:
                currPath = getPath(prime, finalPrime, depth - 1, visited.union(set([startingPrime])), path + [prime], layer + 1)
                if (len(currPath) > 0):
                    if (layer == 0):
...
def main():
    for line in sys.stdin:
        ...
        path = getPath(primes[0].lstrip("0"), primes[1].lstrip("0"), 5)
        if (len(path) > 0):
            print ' '.join(path)
        else:
            print "UNSOLVABLE"
